...
export const Navbar = ({ children, className }) => {
    ...
    return (
        <motion.div ref={ref} className={cn("fixed inset-x-0 top-5 z-[100] w-full px-2", className)}>
...
export const NavBody = ({ children, className, visible }) => {
    return (
        <motion.div
            ...
            className={cn("relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full px-4 py-2 lg:flex bg-black", visible && "bg-neutral-950/80", className)}
        >
            {children}
        </motion.div>
    );
